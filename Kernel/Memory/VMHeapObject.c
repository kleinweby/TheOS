...
static void VMHeapObjectDeallocImpl(VMObject* object);
...
VMHeapObject* VMHeapObjectCreate(VMSection* section, offset_t base, size_t size)
{
	...
	VMHeapObject* obj = kalloc(sizeof(VMHeapObject) + size/kPhyMemPageSize);
	
	if (!obj)
		return NULL;
	
	if (!VMObjectInitialize((VMObject*)obj, section, base, size, VMHeapObjectAddToContextImpl,
...
bool VMHeapObjectExpand(VMHeapObject* obj, size_t size)
{
	...
	if (obj->object.size/kPhyMemPageSize < obj->allocatedPages + pagesNeeded)
		return false;
